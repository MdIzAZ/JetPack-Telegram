package com.kroy.sseditor.presentation.contact_list.remove
...
fun ContactScreen(onAddContact: () -> Unit = {}, onEditClick: (ContactItem) -> Unit = {}, onTransferContact:()->Unit, sharedViewModel: SharedViewModel) {
    ...
    if (contactDetails.value?.data != null && isLoading.value) {
        ...
        var currentTime = addRandomMinutesToTime(SelectedClient.clientTimes.triggerTime, 1, 2)
...
//        )
    ...
    val chatItems = remember { mutableStateListOf<com.kroy.sseditor.domain.models.ContactItem>() } // Initialize as a mutable state list
...
//                clientTimes = "getYesterdaysDateFormatted()", // Get yesterday's date
